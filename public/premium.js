    ...
    fetch(`/api/expenses?page=${currentPage}&limit=${limit}`, {
      ...
      .then(res => res.json()).then(data => {
        ...
        data.expenses.forEach(exp => {
          ...
          li.innerHTML = `
           <div>₹${exp.amount} — ${exp.description} (${exp.category})  <br>
  <div>${exp.date}</div>
    <small>Note: ${exp.note || '-'}</small>
  </div>
  <div class="expense-actions">
    <button data-id="${exp.id}" class="btn-edit edit-btn">✏️ Edit</button>
    <button data-id="${exp.id}" class="btn-delete delete-btn">🗑️ Delete</button>
  </div>`;
